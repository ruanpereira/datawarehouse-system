...
def filter_status_atraso_local(df: pd.DataFrame) -> pd.DataFrame:
    """Retorna apenas vendas com status 'Em atraso'."""
    print(df[df['STATUS COTA'] != 'A'])
    return df[df['STATUS COTA'] != 'A']

def filter_by_year_local(df: pd.DataFrame, year: int) -> pd.DataFrame:
    """Retorna vendas ocorridas no ano especificado."""
    return df[df['DATA VENDA'].dt.year == year]

def total_liquido_por_vendedor_local(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def total_liquido_por_consorcio_vendedor_local(df: pd.DataFrame) -> pd.DataFrame:
...
def relatorio_por_consorciado_local(df: pd.DataFrame) -> dict:
    """
