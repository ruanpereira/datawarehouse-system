...
# 1) Defina o caminho “base” corretamente:
if getattr(sys, 'frozen', False):
    # Quando empacotado pelo PyInstaller:
    # sys._MEIPASS aponta para a pasta temporária onde tudo foi extraído.
    base_path = sys._MEIPASS
else:
    # Em modo desenvolvimento, a raiz é o diretório pai de “data/”
    base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))

# 2) Monte o caminho completo para “.env” dentro de “data/”
dotenv_path = os.path.join(base_path, 'data', '.env')

# 3) Carregue as variáveis de ambiente
load_dotenv(dotenv_path)

# 4) Pegue a string do DATABASE_URL
DATABASE_URL = os.getenv("DATABASE_URL")
if not DATABASE_URL:
    raise RuntimeError(f"Não encontrou DATABASE_URL em {dotenv_path}")

# 5) Crie a engine normalmente
